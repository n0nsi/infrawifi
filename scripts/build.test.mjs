...
async function fixture(t) {
  const dir = await mkdtemp(join(tmpdir(), 'infrawifi-build-test-'));
...
test('reject an allowlisted source redirected through a symlink', async t => {
  const dir = await fixture(t);
  await rm(join(dir, 'assets/img/logo.png'));
  await symlink(join(dir, 'index.html'), join(dir, 'assets/img/logo.png'));
